...
$$Z_t = \varepsilon_t \sqrt{\omega + \alpha \ Z_{t-1}^2}, \ \ \ \ \ t\in \mathbb{Z}$$
where $\varepsilon_t \sim N(0,1)$, $\omega=0.4$, and $\alpha=0.9$.
...
```{r}
sigma_func = function(x){   sqrt(0.4 + 0.9 * x^2)  }
# Zt = N(0,1) * sigma_func(Zt-1)
```
...
simulate_data = function(n_sample, cutoff=0.05){
    ...
    Z[1] = 0
    for (i in 2:n) {
        Z[i] = rnorm(1,0,1) * sigma_func(Z[i-1])
    }
    Z = Z[-(1:burn_in)]
...
theo_con_q = function(x,p){ 
    qnorm(p,0,1,lower.tail=TRUE) * sigma_func(x)
#   qnorm(p,0,sigma_func(x),lower.tail=TRUE)
}
```
...
plot(xx, theo_con_q(xx,1-0.99), col="blue", type="l",
     xlab=expression(X[t]), ylab=" theoretical VaR", main="ARCH model",
...
DMAE_df
write.csv(DMAE_df,'sim_results/ARCH_DMAE_df.csv')
```
...
Z_df
write.csv(Z_df,'sim_results/ARCH_Z_df.csv')
```
...
q_df
write.csv(q_df,'sim_results/ARCH_q_df.csv')
```
...
DMAE_Z_df
write.csv(DMAE_Z_df,'sim_results/ARCH_DMAE_Z_df.csv')
```
...
DMAE_q_df
write.csv(DMAE_q_df,'sim_results/ARCH_DMAE_q_df.csv')
```
...
time_df
write.csv(time_df,'sim_results/ARCH_time_df.csv')
```
