...
$$Z_t = \varepsilon_t \exp(0.5X_t), \ \ X_t = a + b \ X_{t-1}+ c \ \eta_t, \ \ \ \ \ t\in \mathbb{Z}$$
where $\varepsilon_t \sim N(0,1)$, $\eta_t \sim N(0,1)$, $a=0.2$, $b=0.6$, and $c=0.9$.
...
simulate_data = function(n_sample, cutoff=0.05){
    ...
    X[1] = 1
    for (i in 2:n) {
        X[i] = 0.5 + 0.6*(X[i-1]-0.5) + 0.9*rnorm(1,0,1) # 0.2+0.6*X[i-1]+0.9*rnorm(1,0,1)
        Z[i] = exp(0.5*X[i]) * rnorm(1,0,1)
    }
    X = X[-(1:(burn_in+1))]
    Z = Z[-(1:(burn_in+1))]
...
theo_con_q = function(x,p){ 
    exp(0.5*x) * qnorm(p,0,1,lower.tail=TRUE) 
#   qnorm(p,0,exp(0.5*x),lower.tail=TRUE) 
}
```
...
plot(xx, theo_con_q(xx,1-0.99), col="blue", type="l",
     xlab=expression(X[t]), ylab=" theoretical VaR", main="SV model",
     ylim=c(min(theo_con_q(xx,1-0.99)),max(theo_con_q(xx,1-0.95))) )
lines(xx, theo_con_q(xx,1-0.95), col="red")
legend("bottom",legend=c("99% VaR (1% quantile)","95% VaR (5% quantile)"),
...
lines(xx, theo_con_q(xx,1-0.05), col="green")
lines(xx, theo_con_q(xx,1-0.01), col="orange")
legend("topleft",legend=c("99% VaR (1% quantile)","95% VaR (5% quantile)",
                      "5% VaR (95% quantile)","1% VaR (99% quantile)"),
       col=c("blue","red","green","orange"),lty=1, cex=0.8)
...
<span style="color: red;">Question1: So should we care more about the 95% or 99% VaR? </span>
...
DMAE_df
write.csv(DMAE_df,'sim_results/SV_DMAE_df.csv')
```
...
Z_df
write.csv(Z_df,'sim_results/SV_Z_df.csv')
```
...
q_df
write.csv(q_df,'sim_results/SV_q_df.csv')
```
...
DMAE_Z_df
write.csv(DMAE_Z_df,'sim_results/SV_DMAE_Z_df.csv')
```
...
DMAE_q_df
write.csv(DMAE_q_df,'sim_results/SV_DMAE_q_df.csv')
```
...
time_df
write.csv(time_df,'sim_results/SV_time_df.csv')
```
